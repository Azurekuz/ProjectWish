...
Dialogue.prototype.initiate =function(chatSprite, button, pointer, someBool){
	if(this.changeBubble){
		...
		if(this.lineOrderAry[this.i] == "HeadA"){
			...
			newBubble.text = this.game.add.group();
			newBubble.text = this.game.add.text(chatSprite.world.x + chatSprite._frame.width + 14, chatSprite.world.y - 65, this.npc[[parseInt(this.npc[0]) + 1]], { font: "13px Times New Roman", fill: '#000000', wordWrap:true, wordWrapWidth: 304 });
			this.bubbleGroup.add(newBubble);
